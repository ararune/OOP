...
string getWord(string whole, int& i, char& punc) {
	...
	if (isPEQ(whole[i])) { //If a word ends with punctuation, save whatever puctuation it is so it can be appended post-pigLatinize
		if (whole[i] == '.')
			punc = '.';
		else if (whole[i] == '!')
			punc = '!';
...
void pigLatinize(string& word) {
	...
	while (rotations < word.length()) {
		if (isVowel(word[0]) && rotations == 0) { //If the first letter of a word is a vowel
			appHay(word);				  //Append "hay" to the end and break the loop
			break;
		}
		else {
